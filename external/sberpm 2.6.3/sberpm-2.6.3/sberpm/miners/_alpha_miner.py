...
class EncodedUniqueActivities(NamedTuple):
...
class AlphaMiner(AbstractMiner):
    ...
    def apply(self):
        ...
        places = self.find_places(activity_follows_dict, unique_activities)
    ...
    def find_places(self, activity_follows_dict: Mapping[str, set[str]], unique_activities: Iterable[str]):
        """
        ...
        """
        causal_pairs, parallel_pairs = super()._get_causal_parallel_pairs(activity_follows_dict)
        label2act = super()._map_encode_labels_to_activities(unique_activities)

        label_activity_mappings = EncodedUniqueActivities(
            labels_activities=label2act,
            activities_labels={act: label for label, act in label2act.items()},
        )
        independence_matrix = self._create_independence_matrix(
            causal_pairs, parallel_pairs, label_activity_mappings.activities_labels
        )

        return self._calculate(
            causal_pairs,
            independence_matrix,
            label_activity_mappings.activities_labels,
            label_activity_mappings.labels_activities,
            self._n_jobs,
        )
