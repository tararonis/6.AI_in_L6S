...
from typing import Generator, Iterable, NamedTuple, Optional, Union, no_type_check
...
class MandatoryColumns(NamedTuple):
    id_name: str
    activity_name: str
    start_timestamp_name: str
    end_timestamp_name: str
...
class DataHolder:
    ...
    def __init__(
        ...
        self.skip_holidays_list = self.__parse_provided_holidays_list(
            skip_holidays_list
        )
        ...
        if type(data) == str:
            ...
            raise ValueError(
                f"pandas.DataFrame or str types are expected, but got {type(data)}"
            )
        ...
        self.data = (
            self._preprocess_data(
                raw_data, time_format, time_errors, dayfirst, yearfirst
            )
            if preprocess
            else raw_data
        )
    ...
    def id_column(self):
        if self._id_column is not None:
            ...
            raise LogWithoutIdError(
                "The event log does not have an ID column. This action is impossible."
            )

    @property
    def _timestamp_columns(self) -> Iterable[str]:
        return [*filter(None, [self.start_timestamp_column, self.end_timestamp_column])]
    ...
    def __parse_provided_holidays_list(self, provided_holidays_list: deque[str]):
        ...
        for provided_holiday in map(lambda date: date.strip(), provided_holidays_list):
            ...
            min_day, max_day = self.__extract_minmax_days(
                provided_holidays_list, provided_holiday, date_range_idx
            )
            all_holidays.extend(
                self.__extract_holidays_from_range(provided_holiday, min_day, max_day)
            )
    ...
    def __extract_minmax_days(
        ...
        try:
            return map(
                lambda date: match(
                    "\d+", date.strip()
                ).group(),  # extract day number from <day> <month>
                (
                    provided_holiday[:sep_idx],
                    provided_holiday[sep_idx + 1 :],
                ),  # min_date, max_date
            )
        except AttributeError:
            print(
                f"Проверьте правильность заполнения дат для skip_holidays_list: {provided_holidays_list}"
            )
    ...
    def _preprocess_data(self, df, time_format, time_errors, dayfirst, yearfirst):
        ...
        for time_column in [self.start_timestamp_column, self.end_timestamp_column]:
            if time_column is not None and not is_datetime64_any_dtype(time_column):
                if time_format is None:
                    ...
                    dayfirst, yearfirst = DataHolder._check_dayfirst_yearfirst(
                        dayfirst, yearfirst
                    )
        ...
        if self._id_column is not None:
            ...
            if timestamp_col is None:  # no timestamp columns
                ...
                warn(
                    "DataHolder: time column is not given, cannot sort the activities.",
                    UserWarning,
                )
            ...
            if timestamp_col is None:  # no timestamp columns
                raise RuntimeError(
                    "The log that does not have id and any timestamp columns cannot be processed."
                )
            elif self.is_interval():  # 2 timestamp columns
                df = df.sort_values(
                    [self.start_timestamp_column, self.end_timestamp_column]
                )
            else:  # one timestamp column
    ...
    def __filter_out_weekends(self, timestamp_column: str) -> None:
        workday_mask = (
            self.data[timestamp_column].dt.weekday < 5
        )  # 0 stands for Monday, as 5 stand for Saturday
    ...
    def __filter_out_user_holidays(self, timestamp_column: str) -> None:
        ...
        self.data = self.data[
            ~construct_series_of_day_month(self.data[timestamp_column]).isin(
                self.skip_holidays_list
            )
        ]

    def __adjust_calendar_days(self) -> None:
        timestamp_columns_filtered = filter(
            None, (self.start_timestamp_column, self.end_timestamp_column)
        )
    ...
    def get_grouped_data(self, *columns):
        ...
        if len(columns_to_agg) != 0:
            # Group the data
            if self.n_jobs == 1:
                grouped_result = self._groupby(
                    self.data, self.id_column, *columns_to_agg
                )
            else:
                pool = Pool(self.n_jobs)
                result_objects = [
                    pool.apply_async(
                        self._groupby, args=(sub_data, self.id_column, *columns_to_agg)
                    )
                    for sub_data in generate_data_partitions(
                        self.data, self.id_column, batch_num=self.n_jobs * 2
                    )
                ]
            ...
            if self.grouped_data is None:
                self.grouped_data = (
                    grouped_result.reset_index()
                )  # make 'id' a column (not an index)
            else:
                self.grouped_data = self.grouped_data.join(
                    grouped_result, on=self.id_column, how="inner"
                )
    ...
    def _groupby(data, groupby_column, *agg_columns):
        ...
        """
        return data.groupby(groupby_column, as_index=True).agg(
            {col: tuple for col in agg_columns}
        )
    ...
    def check_or_calc_duration(self):
        ...
        """
        if self.duration_column in self.data:
            return
        ...
        if start_timestamp_column is None and end_timestamp_column is None:
            ...
            if (
                self.start_timestamp_column is not None
                and self.end_timestamp_column is None
            ):
    ...
    def is_interval(self) -> bool:
        ...
        return (
            self.start_timestamp_column is not None
            and self.end_timestamp_column is not None
        )
