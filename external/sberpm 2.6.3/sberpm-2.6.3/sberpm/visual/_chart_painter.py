...
class ChartPainter:
    ...
    def hist(
        ...
        **kwargs: optional
        ...
        cols_or_lists = [
            el
            for el in [x, color, list(subplots[:2]) if subplots is not None else []]
            if el is not None
        ]
        data = self._get_data(
            list(chain(*[[el] if isinstance(el, str) else el for el in cols_or_lists]))
        )
        if barmode == "stack":
            barmode = "relative"
        if orientation == "v":
        ...
        if color:
            ...
            color_discrete_sequence = [
                get_continuous_color(self._colorscale, z) for z in nums
            ]
        else:
    ...
    def bar(
        ...
        **kwargs: optional
        ...
        cols_or_lists = [
            ...
            sort,
            color,
            list(subplots[:2]) if subplots is not None else [],
            add_line,
        ]
    ...
    def pie(
        ...
        **kwargs: optional
        ...
        if not values:
            ...
            if n:
                data = data.sort_values(by=values, ascending=False).set_index(labels)[
                    values
                ]
                data_copy = data.copy()
                if n > 0:
        ...
        fig.update_traces(
            sort=False, textinfo=text, insidetextorientation=text_orientation
        )
        if edge:
    ...
    def pareto(
        ...
        if is_numeric_dtype(_data[x]):
            ...
            data = (
                pd_cut(x=_data[x], bins=bins, right=False).value_counts().sort_index()
            )
        ...
        fig.update_traces(
            hovertemplate=f"%{{y:.{decimals}f}}%", selector=dict(type="scatter")
        )
        if title == "auto":
        ...
        fig.update_layout(
            ...
            hovermode="x",
            legend=dict(
                orientation="h", x=0.5, xanchor="center", y=1.01, yanchor="bottom"
            ),
            margin=dict(l=5, r=5, t=80, b=5),
            height=height,
            width=width,
    ...
    def sankey(
        ...
        **kwargs: optional
        ...
        top_traces = (
            TraceMetric(self._dh)
            .calc_metrics(*["count", "ids"])
            .sort_values("count", ascending=False)
            .head(n)
        )
        data_grouped = self._data.groupby(self._dh.id_column, as_index=False).agg(
            {self._dh.activity_column: list}
        )
        data_grouped[self._dh.activity_column] = data_grouped[
            self._dh.activity_column
        ].apply(lambda x: ["start"] + x + ["end"])
        data = data_grouped.apply(Series.explode)
        dh = deepcopy(self._dh)
        dh.data = data[
            data[dh.id_column].isin(list(set.union(*top_traces["ids"].values)))
        ]
    ...
    def _get_data(self, metric_names: Union[str, List[str]]) -> DataFrame:
        ...
        if isinstance(self._data, DataFrame):
            ...
            self._data,
            (ActivityMetric, TransitionMetric, IdMetric, TraceMetric, UserMetric),
        ):
            metric_names = (
                list(set(metric_names))
                if not isinstance(metric_names, str)
                else [metric_names]
            )
            res = self._data.calc_metrics(*metric_names, raise_no_method=False)

            if len(res.columns) == 0:
                res = self._data.calc_metrics(
                    *metric_names, raise_no_method=False
                )  # just for raising error
            ...
            for name in metric_names:
                if name not in res.columns:
                    self._data.calc_metrics(
                        name, raise_no_method=True
                    )  # just for raising error
            return res
        else:
            raise RuntimeError()
    ...
    def hist_activity_of_dur(
        ...
        if top:
            ...
            longest_stages = (
                dh.data.groupby([dh.activity_column])[dh.duration_column]
                .max()
                .sort_values()
                .tail(2)
            )
            ...
            for act in longest_stages.index:
                ...
                list_v = [round(avg_or_median * i / 5.0, round_n) for i in range(1, 10)]
                tmp_data[dh.duration_column][
                    tmp_data[dh.duration_column] > list_v[-1]
                ] = (list_v[-1] + list_v[0] / 2)
            ...
            dh.data[dh.duration_column][dh.data[dh.duration_column] > list_v[-1]] = (
                list_v[-1] + list_v[0] / 2
            )
            ...
            fig.update_layout(
                title=title, height=height, width=width, font_size=font_size
            )
        fig.show(config=self._config)
