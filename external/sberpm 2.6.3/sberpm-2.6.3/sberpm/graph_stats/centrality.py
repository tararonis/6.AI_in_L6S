...
def degree_centrality(G: Graph, degree_type: str = "indirect") -> Dict[str, float]:
    """
    ...
    nodes = [idx for idx, node in G.nodes.items() if node.type not in [NodeType.START_EVENT, NodeType.END_EVENT]]
    if len(nodes) <= 1:
    ...
    if degree_type == "in":
        ...
        raise ValueError('Expected degree_type "in", "out", ' f'or "indirect", but got "{degree_type}".')

    degrees = [degrees[compliance[node]] for node in nodes]
    s = 1.0 / (len(nodes) - 1.0)

    return {n: d * s for n, d in zip(nodes, degrees)}


def eigenvector_centrality(G: Graph, degree_type: str = "indirect") -> Dict[str, float]:
    """
    ...
    norm = sign(largest.sum()) * sc_norm(largest)
