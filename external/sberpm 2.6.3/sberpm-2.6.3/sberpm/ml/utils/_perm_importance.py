...
class PermutationImportance:
    ...
    def fit(self, X, y):
        ...
        self
        """
        # TODO Check and transform the data into np.ndarray
        X = check_array(X)
        ...
        for _ in range(self.n_iter):
            scores_shuffled = array(
                [
                    self._scorer(self.estimator, X_one_col_shuffled, y)
        ...
        self.feature_importances_ = np_mean(scores_diff, axis=0)
        self.feature_importances_std_ = np_std(scores_diff, axis=0)
        return self
    ...
    def explain_weights(self, sort_by_weights=True):
        ...
        df = DataFrame(
            {
                "feature": [f"x{num}" for num in range(len(self.feature_importances_))],
